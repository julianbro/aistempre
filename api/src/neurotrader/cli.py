...
def train(
    ...
    checkpoint: Path | None = typer.Option(None, "--checkpoint", help="Resume from checkpoint"),
    overrides: list[str] | None = typer.Option(None, "--override", "-o", help="Config overrides"),
):
...
def tune(
    backend: str = typer.Option(
        "optuna", "--backend", help="Tuning backend: optuna|ray-pbt|evolutionary"
    ),
    n_trials: int | None = typer.Option(None, "--n-trials", help="Number of trials (Optuna)"),
    time_budget_hours: float | None = typer.Option(
        None, "--time-budget-hours", help="Time budget (Ray PBT)"
    ),
    config_dir: Path = typer.Option("./configs", "--config-dir", help="Config directory"),
):
...
def calibrate(
    ...
    output: Path = typer.Option(
        "calibrated_model.ckpt", "--output", help="Output calibrated checkpoint"
    ),
    method: str = typer.Option(
        "temperature", "--method", help="Calibration method: temperature|isotonic|conformal"
    ),
):
